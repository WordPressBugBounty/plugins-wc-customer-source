...
final class WC_Customer_Source {
  ...
  <?php
	...
	public function display_custom_field() {
		...
		$othertext = __( 'Other', 'wc-customer-source' );

		if ( ! $this->settings['other_field_disable'] ) {
			$source_options['other'] = $othertext;
		}
		?>
	 ...
	 public function display_fields_order_edit( $order ) {
		 ...
		 $source = esc_html(get_post_meta( $order->get_id(), 'wc_customer_source_checkout_field', true ));
		 $other  = esc_html(get_post_meta( $order->get_id(), 'wc_customer_source_checkout_other_field', true ));
